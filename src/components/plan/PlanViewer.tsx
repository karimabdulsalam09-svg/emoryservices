...
import { Settings, Palette, FileText, Rocket, Wrench } from "lucide-react";
...
type PlanView = "build" | "launch";

const PlanViewer = ({ plan }: { plan: ClientPlan }) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const [activeView, setActiveView] = useState<PlanView>("build");
    ...
    <div ref={containerRef} className="max-w-4xl mx-auto px-4 md:px-6">
      ...
      >
        ...
        <p className="text-muted-foreground text-sm md:text-lg max-w-xl mx-auto">
          Your personalised launch plan is ready. Switch between the Build Plan and the Launch Plan below.
        </p>
      </motion.div>
      ...
      {hasFrontEndPlan && (
        <div className="flex justify-center mb-10 md:mb-14">
          <div className="glass-card inline-flex p-1.5 rounded-2xl gap-1.5">
            <button
              onClick={() => setActiveView("build")}
              className={`flex items-center gap-2 px-4 md:px-6 py-2.5 md:py-3 rounded-xl text-xs md:text-sm font-semibold transition-all duration-300 ${
                activeView === "build"
      ...
      <AnimatePresence mode="wait">
        {activeView === "build" ? (
          ...
          >
            {/* Role legend */}
            <div className="flex justify-center gap-6 text-sm text-muted-foreground mb-10 md:mb-14">
            ...
            {plan.plan_data.length > 0 && (
              ...
              >
                ...
                <h2 className="text-2xl md:text-3xl font-bold mb-2">Build Complete!</h2>
                <p className="text-muted-foreground mb-4">
                  You've completed the 14-day build plan. Ready to launch?
                </p>
                {hasFrontEndPlan && (
                  <button
                    onClick={() => {
                    ...
                    className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-accent text-accent-foreground font-semibold text-sm hover:bg-accent/80 transition-colors"
                  >
