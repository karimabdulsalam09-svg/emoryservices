              ...
              {workflowSteps.map((step, index) => (
                <div key={index} className="flex items-center gap-2">
                  ...
                  {index < workflowSteps.length - 1 && (
                    <motion.svg width="60" height="40" viewBox="0 0 60 40" className="flex-shrink-0" initial={{ opacity: 0, scaleX: 0 }} whileInView={{ opacity: 1, scaleX: 1 }} viewport={vp} transition={{ duration: 0.5, delay: 0.3 + index * 0.2 }} style={{ transformOrigin: "left" }}>
                      <path d="M0,20 C15,5 45,35 60,20" stroke="hsl(187 100% 50% / 0.35)" strokeWidth="2" fill="none" strokeDasharray="4 3" />
                    </motion.svg>
                  )}
                </div>
              ))}
            ...
            <div className="md:hidden space-y-2">
              {workflowSteps.map((step, index) => (
                <div key={index} className="flex flex-col items-center">
                  ...
                  {index < workflowSteps.length - 1 && (
                    <motion.svg width="40" height="30" viewBox="0 0 40 30" initial={{ opacity: 0, scaleY: 0 }} whileInView={{ opacity: 1, scaleY: 1 }} viewport={vp} transition={{ duration: 0.3, delay: index * 0.1 + 0.15 }} style={{ transformOrigin: "top" }}>
                      <path d="M20,0 C5,10 35,20 20,30" stroke="hsl(187 100% 50% / 0.35)" strokeWidth="2" fill="none" strokeDasharray="4 3" />
                    </motion.svg>
                  )}
                </div>
              ))}
